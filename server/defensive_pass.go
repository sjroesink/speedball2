...
func passDirection(p *Player, x, z float64) int {
	const unit = 22.4 / 576
	dx, dz := int(math.Round(x/unit)-math.Round(p.X/unit)), int(math.Round(z/unit)-math.Round(p.Z/unit))
	fx, fz := 0, 0
	if absInt(dx) > absInt(dz)/2 {
...
func (s *State) opponentDirections(i int, distances *[18]int) [2]int {
	p := &s.Players[i]
	aim := func(j int) int {
		q := &s.Players[j]
		x, z := predictedTarget(q.X, q.Z, q.moveX, q.moveZ, p.Stats[7])
		return passDirection(p, x, z)
	}
